...
public class Manager extends Agent {
			...
			if(encontrou==0) 
			{
				for(int x=0;x<pos-1;x++)
				{
					...
					if(gs1.getJogador(result[x].getName().getLocalName()).getBribe()==-1 && !gs1.getJogador(result[x].getName().getLocalName()).equals(gs1.canalOverseerName))
					{
						...
						try {
							ACLMessage msgBRIBE = new ACLMessage(ACLMessage.INFORM);
	...
	public void gameOver() throws IOException
	{
		...
		{
			//System.out.println(stats.get(i).getNome() + " - " + gs1.getValorTotalJogador(stats.get(i).getNome()));
			if(gs1.getValorTotalJogador(stats.get(i).getNome()) > max)
			{
				jogador = stats.get(i).getNome();
				max = gs1.getValorTotalJogador(stats.get(i).getNome());
			}
		}
		
		String winner = jogador + " : " + max;
		mainFrame.mainpanel.print_Winner(gs1,winner);
